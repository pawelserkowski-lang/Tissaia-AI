...
const FileListView: React.FC<FileListViewProps> = ({ files, isLoading, onUpload, onSelect, onDelete, onClear, onRetry, onVerify }) => {
  ...
  // Local state for inputs before verification (Table View)
  ...
  const openPreview = (file: ScanFile) => {
      setPreviewFile(file);
      // Initialize modal input with existing expected count or blank
      ...
      >
        ...
        <div className="overflow-x-auto h-full custom-scrollbar">
            <table className="w-full text-left text-sm text-gray-400">
                {/* MODIFIED: Increased font size for header to text-sm */}
                <thead className="bg-white/5 text-gray-200 uppercase font-mono text-sm tracking-wider sticky top-0 z-10 backdrop-blur-sm">
                    <tr>
                        ...
                        <th className="px-6 py-5 border-b border-white/5 w-40">Podgląd (Kliknij)</th>
                        ...
                        <>
                            {filteredFiles.map((file) => (
                                <tr key={file.id} className={`hover:bg-white/5 transition-colors group ${selectedIds.has(file.id) ? 'bg-white/5' : ''}`}>
                                    ...
                                    {/* THUMBNAIL COLUMN - INCREASED SIZE */}
                                    <td className="px-6 py-6 cursor-zoom-in" onClick={() => openPreview(file)}>
                                        <div className="w-32 h-32 bg-black/40 rounded-xl flex items-center justify-center text-gray-600 overflow-hidden border border-white/10 group-hover:border-tissaia-accent/50 transition-all shadow-lg group-hover:shadow-[0_0_15px_rgba(0,255,163,0.15)] relative">
                                            {file.thumbnailUrl ? (
                                    ...
                                    {/* FILENAME COLUMN - INCREASED FONT to text-2xl */}
                                    <td className="px-6 py-6 font-medium text-gray-200 cursor-pointer" onClick={() => file.status !== ScanStatus.UPLOADING && onSelect(file.id)}>
                                        <div className="text-2xl font-bold text-white tracking-wide mb-2 group-hover:text-tissaia-accent transition-colors">
                                            {file.filename}
                                        </div>
                                        {/* MODIFIED: Increased metadata font to text-sm */}
                                    ...
                                    <td className="px-6 py-6 min-w-[200px]">
                                        {file.status === ScanStatus.UPLOADING ? (
                                            ...
                                            // MODIFIED: Total War Progress Bar
                                            <div className="w-full">
                                                <div className="flex justify-between mb-1">
                                        ...
                                        {file.status === ScanStatus.PENDING_VERIFICATION ? (
                                            <div className="flex items-center justify-center space-x-2">
                                                <input 
                                                    ...
                                                    // MODIFIED: Increased input font to text-2xl
                                    ...
                                    <td className="px-6 py-6 text-center font-mono text-white font-bold text-2xl">
        ...
        {previewFile && (
            <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-8 animate-fade-in">
                <div className="w-full h-full max-w-7xl flex flex-col md:flex-row gap-8">
                    ...
                    {/* Controls Panel */}
                    <div className="w-full md:w-96 flex flex-col justify-center space-y-6 shrink-0">
